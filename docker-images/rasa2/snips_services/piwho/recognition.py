...
class SpeakerRecognizer(object):
    ...
    def train_new_data(self, filepath=None, speakername=None):
        ...
        if filepath is not None and os.path.isdir(filepath):
            ...
            execCommand = 'java ' + config.JAVA_MEM + '-jar ' + config.SPEAKER_RECOG_JAR + ' --train ' + filepath + self.feature
            #print (execCommand)
            reply = self._start_subprocess(execCommand)
            #print("REPLIED {}".format(reply))
    ...
    def identify_speaker(self, audiofile=None):
        ...
        command = 'java ' + config.JAVA_MEM + '-jar ' + config.SPEAKER_RECOG_JAR +' --ident ' + expand + self.feature
        print(command)
        # handle this error
        name = self._start_subprocess(command)
        print(name)
        self.scores = name[4:]
        if not self.debug:
            try:
    ...
    def _is_good_wave(self, filename):
        ...
        """
        #print("is good wav ? {}".format(filename))
